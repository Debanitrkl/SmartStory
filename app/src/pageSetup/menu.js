...
<div class="ui borderless small top menu page-menu" 
  ...
  <div class="item" style="height: 100%; display: flex; align-items: center; justify-content: center; color: rgba(0,0,0,.6);">
    <h2 class="special-comic-text">Smart Story</h2>
  </div>
...
Vue.component('page-menu', {
  ...
  mounted: function () {
    let vm = this;
    $.post('./getDataFile').then(function (dataFile) {
      window.dataFile = dataFile;
      if (window.dataFile != '') {
        $('#dataFileSelection').dropdown(
          'set selected',
          vm.dataPathToDropdownDataFile(window.dataFile)
        );
      }
      $('#dataFileSelection').dropdown({
